...
export class MonadaEditComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    console.log(newLegalProvisions);
  ...
  updateRemitTextWithNewProvision(newText: string) {
    ...
    if (!(this.legalProvisions.length === 1 && ('isNew' in this.legalProvisions[0]))) {
      this.showInfoText = "<p style='color:red'>Στο πάνω μέρος του Κειμένου Πρόβλεψης της Σύστασης της Μονάδας, εμφανίζεται το κείμενο της τελευταίας διάταξης που έχετε εισάγει. Στο κάτω μέρος εμφανίζεται το προγενέστερο κείμενο <strong>ως είχε πριν την τελευταία τροποποίηση</strong>. Επεξεργαστείτε και κωδικοποιήστε το κείμενο της πρόβλεψης για τη σύσταση της Μονάδας <strong>όπως ισχύει ενιαία με την τελευταία τροποποιητική διάταξη</strong>.</p>";
    }

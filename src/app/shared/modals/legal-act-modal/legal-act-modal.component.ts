...
import { NgbAlertModule, NgbDatepickerModule, NgbInputDatepicker, NgbTooltipModule, NgbDatepickerI18n, NgbDateParserFormatter, NgbDateStruct } from '@ng-bootstrap/ng-bootstrap';
...
import { NgbDateELParserFormatter, NgbDateELI18n } from 'src/app/shared/services/greek-date-locale.service';
...
@Component({
  ...
  providers: [
    { provide: NgbDateParserFormatter, useClass: NgbDateELParserFormatter },
    { provide: NgbDatepickerI18n, useClass: NgbDateELI18n }
  ]
})
export class LegalActModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formSubscriptions.push(
      this.form.get('fek').valueChanges.subscribe(value => {
        ...
        const date = moment(this.toIso(value.date), 'YYYY-MM-DD', true);
        if (date.isValid()) {
          const year = date.year().toString();
          const legalActType = this.form.get('legalActType').value;
          if (
            legalActType &&
            ['ΝΟΜΟΣ', 'ΠΡΟΕΔΡΙΚΟ ΔΙΑΤΑΓΜΑ'].includes(legalActType) &&
            !this.emptyFEK()
          ) {
  ...
  onSubmit() {
    ...
    if (data.fek.date != '') data.fek.date = this.toIso(this.form.get('fek.date').value);
  ...
  formValid(): boolean {
    ...
    if (legalActType && ['ΝΟΜΟΣ', 'ΠΡΟΕΔΡΙΚΟ ΔΙΑΤΑΓΜΑ'].includes(legalActType)) {
      if (this.emptyFEK()) {
        ...
        //   this.form.valid,
        //   this.fekYear,
        //   this.form.get('legalActDateOrYear').value,
        // );
  ...
  private toIso(date: NgbDateStruct | null): string | null {
    if (!date) return null;
